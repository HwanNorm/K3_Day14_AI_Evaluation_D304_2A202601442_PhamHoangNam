...
class LLMJudge:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        positional_bias = bool(
            ...
            and _avg(first_scores) > _avg(later_scores) + 0.2
        )
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        avg_context_recall = _avg(recall_values) if recall_values else None
        avg_context_precision = _avg(precision_values) if precision_values else None
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        low_scores = [name for name, value in scores.items() if value < 0.5]
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination"):
            suggestions.append(
                ...
                "and tighten grounding guardrails"
            )
        if categories.get("irrelevant"):
            suggestions.append(
                "Improve prompt clarity and query understanding so answers "
                "directly address the question intent"
            )
        if categories.get("incomplete"):
            suggestions.append(
                "Increase chunk size or retrieval top-k to reduce context "
                "fragmentation and capture missing details"
            )
        if categories.get("off_topic"):
            suggestions.append(
                "Review intent detection and routing logic to prevent "
                "off-topic responses"
            )
        ...
        while len(suggestions) < 3:
            fallback = [
                ...
                "Expand the evaluation golden dataset to cover more edge cases",
                "Introduce a retrieval reranker to surface the most relevant "
                "chunks earlier",
            ]
